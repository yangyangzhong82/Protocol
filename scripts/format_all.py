import os
import subprocess
from concurrent.futures import ThreadPoolExecutor
import multiprocessing
import threading
import argparse


FILE_COUNT_PER_TASK = 50
lock = threading.Lock()
formatted_count = 0
total_count = 0

silent = False


def parse_arguments():
    parser = argparse.ArgumentParser(description="clang-format")
    parser.add_argument("--silent", action="store_true")
    return parser.parse_args()


def finish_count(count):
    global formatted_count
    with lock:
        formatted_count += count
        return formatted_count


def get_all_code_files(directory, extensions):
    code_files = []
    for root, _, files in os.walk(directory):
        for file in files:
            if file.endswith(extensions):
                code_files.append(os.path.join(root, file))
    return code_files


def format_file(file_paths, clang_format_path, silent=False):
    args = [clang_format_path, "-i"]
    for file_path in file_paths:
        args.append(file_path)
    try:
        subprocess.run(args, check=True)
        subprocess.run(args, check=True)
        msg = f"Formatted: {[os.path.basename(file) for file in file_paths]}"
        if len(msg) > 50:
            msg = msg[:50] + "..."
        count = finish_count(len(file_paths))
        if not silent:
            print(f"{msg} ({count}/{total_count})")
    except subprocess.CalledProcessError as e:
        if not silent:
            print(f"Failed to format {file_paths}: {e}")


def convert_line_endings(input_file, output_file):
    with open(input_file, "r", encoding="utf-8") as infile:
        content = infile.read()

    content = content.replace("\r\n", "\n").replace("\n", "\r\n")

    with open(output_file, "w", encoding="utf-8", newline="") as outfile:
        outfile.write(content)


def format_code_files(
    directory,
    clang_format_path,
    silent=False,
    extensions=(".cpp", ".c", ".h", ".hpp"),
    threads=None,
):
    global total_count
    code_files = get_all_code_files(directory, extensions)
    total_count = len(code_files)
    if not silent:
        print(f"Found {len(code_files)} files to format.")

    if threads is None:
        threads = max(1, multiprocessing.cpu_count() - 2)
        if not silent:
            print(f"Using {threads} threads (number of CPU cores).")

    with ThreadPoolExecutor(max_workers=threads) as executor:
        for i in range(0, len(code_files), FILE_COUNT_PER_TASK):
            file_batch = code_files[i : i + FILE_COUNT_PER_TASK]
            executor.submit(format_file, file_batch, clang_format_path, silent)


if __name__ == "__main__":
    clang_format_path = r"clang-format"

    args = parse_arguments()
    silent = args.silent

    import time

    if not silent:
        print(f"Started at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
    format_code_files("./src", clang_format_path, silent)
    format_code_files("./include", clang_format_path, silent)
    if not silent:
        print(f"Finished at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
